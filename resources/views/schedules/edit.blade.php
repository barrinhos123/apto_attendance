    ...
    <div class="py-12">
        <div class="max-w-2xl lg:max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg border border-slate-200 dark:border-slate-700">
                <form method="POST" action="{{ route('attendance.schedules.update', $schedule) }}" class="p-6 space-y-4" id="schedule-form">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-label for="name" value="{{ __('Name') }}" />
                        <x-input id="name" type="text" name="name" class="mt-1 block w-full" :value="old('name', $schedule->name)" required autofocus />
                        <x-input-error for="name" class="mt-2" />
                    </div>

                    @php
                        $hours = old('duration_hours', (int) floor($schedule->duration_minutes / 60));
                        $minutes = old('duration_minutes', $schedule->duration_minutes % 60);
                    @endphp
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <x-label for="duration_hours" value="{{ __('Duration') }}" />
                            <div class="mt-1 flex gap-3 items-center">
                                <div class="flex items-center gap-2">
                                    <x-input id="duration_hours" type="number" name="duration_hours" min="0" max="24" :value="$hours" class="w-20" required />
                                    <span class="text-sm text-slate-600 dark:text-slate-400">{{ __('h') }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <x-input id="duration_minutes" type="number" name="duration_minutes" min="0" max="59" :value="$minutes" class="w-20" required />
                                    <span class="text-sm text-slate-600 dark:text-slate-400">{{ __('min') }}</span>
                                </div>
                            </div>
                            <x-input-error for="duration_hours" class="mt-2" />
                            ...
                            @foreach(\Apto\Attendance\Models\Schedule::weekdays() as $key => $label)
                                <label class="inline-flex items-center gap-2">
                                    <input type="checkbox" name="days_of_week[]" value="{{ $key }}" class="rounded border-slate-300 dark:border-slate-600 text-indigo-600 focus:ring-indigo-500 dark:bg-slate-700"
                                        {{ in_array($key, old('days_of_week', $schedule->days_of_week ?? [])) ? 'checked' : '' }}>
                            ...
                            @foreach(\Apto\Attendance\Models\Schedule::locationTypes() as $value => $label)
                                <label class="inline-flex items-center gap-2">
                                    <input type="radio" name="location_type" value="{{ $value }}" class="border-slate-300 dark:border-slate-600 text-indigo-600 focus:ring-indigo-500 dark:bg-slate-700"
                                        {{ old('location_type', $schedule->location_type) === $value ? 'checked' : '' }}>
                                    <span class="text-sm text-slate-700 dark:text-slate-300">{{ $label }}</span>
                                </label>
                            @endforeach
                        </div>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Outside schedules can have a specific place.') }}</p>
                    ...
                    <div id="map" class="w-full h-64 sm:h-72 md:h-96 lg:h-[28rem] xl:h-[32rem] rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-700" aria-hidden="true"></div>

                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="sm:col-span-2">
                            <x-label for="location" value="{{ __('Address / location') }}" />
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <x-input id="location" type="text" name="location" class="block w-full rounded-r-none" :value="old('location', $schedule->location)" placeholder="{{ __('e.g. 123 Main St, City') }}" />
                        ...
                        <div>
                            ...
                            <x-input id="latitude" type="text" name="latitude" class="mt-1 block w-full" :value="old('latitude', $schedule->latitude)" placeholder="e.g. 40.7128" inputmode="decimal" />
                            <x-input-error for="latitude" class="mt-2" />
                        </div>
                        <div>
                            <x-label for="longitude" value="{{ __('Longitude') }}" />
                            <x-input id="longitude" type="text" name="longitude" class="mt-1 block w-full" :value="old('longitude', $schedule->longitude)" placeholder="e.g. -74.0060" inputmode="decimal" />
