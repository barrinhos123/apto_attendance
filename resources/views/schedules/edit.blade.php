        ...
        <div class="max-w-2xl lg:max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg border border-slate-200 dark:border-slate-700">
                <form method="POST" action="{{ route('attendance.schedules.update', $schedule) }}" class="p-6 space-y-4" id="schedule-form">
                    ...
                    @php
                        $clockIn = old('clock_in_time', $schedule->clock_in_formatted);
                        $clockOut = old('clock_out_time', $schedule->clock_out_formatted);
                    @endphp
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" x-data="{
                        clockIn: @js($clockIn),
                        clockOut: @js($clockOut),
                        get duration() {
                        ...
                        <div>
                            ...
                            <x-input id="clock_in_time" type="time" name="clock_in_time" class="mt-1 block w-full" :value="$clockIn" x-model="clockIn" required />
                            <x-input-error for="clock_in_time" class="mt-2" />
                        </div>
                        <div>
                            <x-label for="clock_out_time" value="{{ __('Clock out time') }}" />
                            <x-input id="clock_out_time" type="time" name="clock_out_time" class="mt-1 block w-full" :value="$clockOut" x-model="clockOut" required />
                    ...
                    <div x-data="{ locationType: @js(old('location_type', $schedule->location_type)) }">
                        <div>
